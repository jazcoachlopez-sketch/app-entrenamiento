...
# Inicializamos la variable de control de clics si no existe
...
if opcion == "📝 Registrar Entrenamiento":
    ...
    # BOTÓN DE GUARDADO ANTIDUPLICADOS
    texto_boton = "⌛ Guardando entrenamiento..." if st.session_state.guardando_entrenamiento else "🚀 Guardar Entrenamiento"
    
    if st.button(texto_boton, on_click=desactivar_boton, disabled=st.session_state.guardando_entrenamiento):
        if not atleta_input or atleta_input == "":
            ...
            try:
                ...
                # ESCUDO 2: Validar si ya existe el registro exacto en el Sheets para evitar doble envío por red
                ...
                if es_duplicado:
                    st.warning("⚠️ Este entrenamiento ya fue registrado hace unos instantes. No se guardará un duplicado.")
                    time.sleep(2)
                else:
                    nuevo_reg = {
                        "Fecha": [fecha_str], 
                ...
                # Restauramos el estado del botón y recargamos la app
                st.session_state.guardando_entrenamiento = False
                st.rerun()
