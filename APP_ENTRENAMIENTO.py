    ...
    if st.session_state.guardando_entrenamiento:
        ...
        if not atleta_input or atleta_input == "":
            ...
            try:
                ...
                if es_duplicado:
                    st.warning("⚠️ Este entrenamiento ya fue registrado hace unos instantes. No se creará un duplicado.")
                    time.sleep(2)
                else:
                    nuevo_reg = {
                        "Fecha": [fecha_str], 
            ...
            # Liberamos el seguro del botón y refrescamos la interfaz limpia
            st.session_state.guardando_entrenamiento = False
            st.rerun()
    else:
        # Botón original en modo activo
